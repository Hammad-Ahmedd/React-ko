...
function App() {
  ...
  const [scoreY, setscoreO] = useState(0)
  ...
  useEffect(() => {
    for (let item of winPatterns) {
      ...
      if (p1 !== "" && p1 === p2 && p2 === p3) {
        ...
        if (p1 === "X") {
          setscoreX(pre => pre + 1)
          localStorage.setItem("scoreX", setscoreX);
        } else {
          setscoreY(pre => pre + 1)
          localStorage.setItem("scoreO", scoreY);
        }
      }
    ...
    <>
      <div className='tictactoe-wrapper'>
        <div className="score-wrapper">
          <p>O score <span style={{ color: "red" }} >{scoreY}</span></p>
          <p>X score <span style={{ color: "blue" }} >{scoreX}</span></p>
        </div>
