...
function App() {
  ...
  useEffect(() => {
    const totalPrice = useselect.reduce(
    ...
    <div className="containr-large">
      <>
        ...
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black/25 p-5" style={{ display: sitcher ? "flex" : "none" }}>
            <div className="relative flex flex-col gap-4 bg-white rounded-lg overflow-auto w-[100%] max-w-lg max-h-[90vh] p-6 md:w-[70%] lg:w-1/2 md:p-4">
              <p className="absolute right-6 top-2 -rotate-45 text-4xl cursor-pointer" onClick={() => switches(cardSwitcher())} >+</p>
              ...
              {useselect.length >= 1 && <p style={{ fontSize: "1rem" }} >Total Amount: ${Total}</p>}
            </div>
          </div>
        </div>
      </>
